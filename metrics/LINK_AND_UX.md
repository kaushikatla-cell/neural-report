...
- **Site pages** (home, about, archive, …): `images/og-default.png` at **1200×630** (regenerated by `scripts/generate_og_images.py`).
- **Evidence Briefs**: `images/og/{slug}.png` at **1200×630**, with `og:image` / `twitter:image` / JSON-LD `image` synced by the same script from `scripts/site-manifest.json`.
- After changing the manifest, run **`python3 scripts/generate_og_images.py`** and commit updated PNGs + any HTML OG meta drift CI reports.
